...
async def list_cases(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Case).where(Case.user_id == current_user.id).order_by(Case.created_at.desc()))
...
async def run_eligibility(case_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Case).where(Case.id == uuid.UUID(case_id), Case.user_id == current_user.id))
    case = result.scalar_one_or_none()
    if not case:
        raise HTTPException(404, "Case not found")
...
async def run_risk(case_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Case).where(Case.id == uuid.UUID(case_id), Case.user_id == current_user.id))
    case = result.scalar_one_or_none()
    if not case:
        raise HTTPException(404, "Case not found")
    ...
    case_data = {
        "user_profile": {"nationality": current_user.nationality, "education_level": current_user.education_level},
...
async def list_checklist(case_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Case).where(Case.id == uuid.UUID(case_id), Case.user_id == current_user.id))
    case = result.scalar_one_or_none()
    if not case:
        raise HTTPException(404, "Case not found")
    ...
    checklist_result = await db.execute(
        ...
        .where(UserChecklist.case_id == case.id)
...
async def update_checklist_item(
    ...
    case_result = await db.execute(select(Case).where(Case.id == uuid.UUID(case_id), Case.user_id == current_user.id))
    ...
    item_result = await db.execute(
        select(UserChecklist)
        .where(UserChecklist.id == uuid.UUID(checklist_id), UserChecklist.case_id == case.id)
    )
