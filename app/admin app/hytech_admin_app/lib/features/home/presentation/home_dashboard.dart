...
import '../../appointments/presentation/meetings_screen.dart';
...
class _HomeDashboardState extends State<HomeDashboard> {
  ...
  int _analyticsRefreshTrigger = 0;
    ...
    return Scaffold(
      ...
      body: IndexedStack(
        ...
        children: [
          ...
          _LeadsTab(onLeadTap: (student) => Navigator.push(context, MaterialPageRoute(builder: (_) => LeadDetailsScreen(student: student)))),
          const MeetingsScreen(),
          AnalyticsScreen(refreshTrigger: _analyticsRefreshTrigger),
      ...
      floatingActionButton: _currentIndex == 1
          ? FloatingActionButton(
              ...
              onPressed: () async {
                ...
                bloc.add(AdminLoadLeadsRequested());
              },
          ...
          onTap: (i) {
            setState(() {
              ...
              if (i == 0) {
                ...
                context.read<AdminStudentBloc>().add(AdminLoadLeadsRequested());
              } else if (i == 2) {
                context.read<MeetingsBloc>().add(MeetingsLoadRequested());
              } else if (i == 3) {
                _analyticsRefreshTrigger++;
              }
            });
          },
          items: const [
            ...
            BottomNavigationBarItem(icon: Icon(Icons.event_outlined), activeIcon: Icon(Icons.event), label: 'Meetings'),
...
class _DashboardTab extends StatefulWidget {
  ...
  const _DashboardTab({required this.onTabChange, this.refreshTrigger = 0});
...
class _DashboardTabState extends State<_DashboardTab> {
  ...
  Widget build(BuildContext context) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Failed to load dashboard metrics: $_error', style: const TextStyle(fontWeight: FontWeight.bold), textAlign: TextAlign.center),
    ...
    final whatsappSent = _stats!['whatsapp_sent_count'] ?? 142;
    final actionReq = _stats!['action_required'] ?? {'name': 'No Urgent Leads', 'detail': 'All clean'};
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Text('$leadsCount Active Leads  •  $pendingDocs Docs Pending', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 12, color: AppColors.onSurface)),
                          ),
                        ],
                      ),
                    ),
            ...
            child: Column(
              children: [
                Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Container(width: 48, height: 48, decoration: BoxDecoration(color: AppColors.tertiaryContainer.withAlpha(100), shape: BoxShape.circle), alignment: Alignment.center, child: Text(actionReq['name'].toString().split(' ').map((e) => e.isNotEmpty ? e[0] : '').take(2).join().toUpperCase(), style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.tertiary, fontSize: 16))),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(actionReq['name'] ?? 'Unknown Student', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w700, color: AppColors.onSurface)),
                          Text(actionReq['detail'] ?? 'Visa Checklist Pending', style: const TextStyle(fontSize: 13, color: AppColors.onSurfaceVariant), overflow: TextOverflow.ellipsis),
                          const SizedBox(height: 6),
                          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFFEF08A), borderRadius: BorderRadius.circular(12)), child: const Text('URGENT', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Color(0xFF854D0E)))),
                        ],
                      ),
                    ),
                  ],
                ...
                const SizedBox(height: 12),
                Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppColors.surfaceContainerLow, borderRadius: BorderRadius.circular(8), border: Border.all(color: AppColors.surfaceVariant)), child: Row(children: const [Icon(Icons.info_outline, size: 16, color: AppColors.onSurfaceVariant), SizedBox(width: 8), Expanded(child: Text('Bank statements or passports uploaded recently need manual verification.', style: TextStyle(fontSize: 12, color: AppColors.onSurfaceVariant)))])),
                const SizedBox(height: 12),
                Row(
                  children: [
                    Expanded(
                      child: ElevatedButton(
                        ...
                        child: const Text('VERIFY DOCUMENTS', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis),
                      ),
                    ),
                    const SizedBox(width: 12),
                    SizedBox(
                      ...
                      child: OutlinedButton(
                        style: OutlinedButton.styleFrom(side: const BorderSide(color: AppColors.surfaceVariant, width: 2), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), padding: EdgeInsets.zero),
                        onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CommsLogScreen())),
                        child: const Icon(Icons.message_outlined, color: AppColors.onSurface),
                      ),
                    ),
                  ],
                ),
          ...
          _card(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('OCR Engine extracted data from new passports. Ready to sync with CRM.', style: TextStyle(color: AppColors.onSurface, fontSize: 14)),
                const SizedBox(height: 12),
                ElevatedButton.icon(
                  ...
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('AI OCR Pipeline synchronized successfully!'),
                        backgroundColor: AppColors.primary,
                      ),
                    );
                  },
          ...
          _card(
            child: Column(
              children: logs.asMap().entries.map((e) {
                ...
                if (log['icon'] == 'person_add') {
                  logIcon = Icons.person_add;
                  bg = const Color(0xFFD1FAE5);
                  iconColor = AppColors.primary;
                } else if (log['icon'] == 'folder') {
                  logIcon = Icons.folder_open;
...
class _LeadsTab extends StatefulWidget {
  final Function(UserModel student) onLeadTap;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AdminStudentBloc, AdminStudentState>(
      builder: (context, state) {
        final bloc = context.read<AdminStudentBloc>();
        final leads = bloc.leads;

        if (state is AdminStudentLoading && leads.isEmpty) {
          return const Center(child: CircularProgressIndicator(color: AppColors.primary));
        }

        if (state is AdminStudentError && leads.isEmpty) {
          return Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Text('Error: ${state.message}', style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.error)),
                const SizedBox(height: 12),
                ElevatedButton(
                  onPressed: () => context.read<AdminStudentBloc>().add(AdminLoadLeadsRequested()),
                  child: const Text('Retry'),
                ),
              ],
            ),
        ...
        if (leads.isEmpty) {
          return const Center(
            child: Padding(
              ...
              child: Text('No registered leads found in database.', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.outline)),
            ),
          );
        }
          return Column(
            children: [
              Padding(
                ...
                child: Container(
                  ...
                  child: TextField(
                    decoration: InputDecoration(hintText: 'Search leads by name, email...', hintStyle: TextStyle(color: AppColors.onSurfaceVariant.withAlpha(128)), prefixIcon: const Icon(Icons.search, color: AppColors.onSurfaceVariant), border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14)),
                  ),
                ),
              ),
              const SizedBox(height: 12),
                ...
                child: RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final initials = lead.fullName.split(' ').map((e) => e[0]).take(2).join().toUpperCase();
                      final visaType = lead.educationLevel ?? 'Student Visa';
                      final countryFlag = lead.nationality == 'US' ? '🇺🇸' : (lead.nationality == 'GB' ? '🇬🇧' : (lead.nationality == 'CA' ? '🇨🇦' : '🇮🇳'));
                      return Padding(
                        ...
                        child: _LeadCard(
                          ...
                          visa: visaType,
                          flag: countryFlag,
                          status: lead.isVerified ? 'Verified' : 'New Intake',
                          color: lead.isVerified ? AppColors.primary : AppColors.secondary,
                          progress: lead.isVerified ? 1.0 : 0.5,
                          updated: 'Just now',
...
class _KpiCard extends StatelessWidget {
  ...
  const _KpiCard({required this.title, required this.value, required this.icon, required this.color, required this.subtitle, this.onTap});
